...
This repository is GWR-owned. It contains GWR code, data, deployment workflows, measurement logic, and data utilities only.

Stratum, Vector Praxis, Digital Index Base, and other brands are not runtime dependencies. They may consume verified GWR signals through explicit handoff or public links, but GWR continues to operate without them.
...
Independent global-work intelligence service focused on Japan-based users.
...
```bash
node agent-lab-handoff.mjs --validate-log
node agent-lab-handoff.mjs --record path/to/evidence-backed-candidate.json
```
...
```bash
node publishing-handoff.mjs --validate-log
node publishing-handoff.mjs --record path/to/evidence-backed-data-insight.json
```
