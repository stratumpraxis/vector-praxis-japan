...
The issued Vercel URL is **not yet an acceptable public production origin**. An unauthenticated request currently receives a `302` to Vercel SSO with `x-robots-tag: noindex`. The proposed stable alias has not been assigned. Therefore canonical, sitemap, and robots remain pointed at the public Sites origin.
...
- Submitted the project to Vercel as a production deployment.

## Smallest remaining human action

1. Open the Vercel inspector link above while signed in to the connected `stratumpraxis-7205s-projects` team.
2. Confirm the deployment belongs to the intended project and completed successfully.
3. In **Project Settings → Deployment Protection**, disable Vercel Authentication for the production environment. Do not replace it with password protection.
4. In **Project Settings → Domains**, confirm or assign a stable free production alias ending in `.vercel.app`.
5. Share the stable alias so the final canonical transition and public QA can be completed.
