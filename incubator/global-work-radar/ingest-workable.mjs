...
const stripHtml = (value = '') => decode(value.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' '));
...
const inferEligibility = ({ country, text }) => {
  ...
  if (/\b(japan|jp)\b|日本/.test(t)) {
    return { japan_eligible: true, confidence: 0.9, eligible_region: 'Japan', evidence: 'Explicit Japan signal in source facts.' };
  }
  if (/worldwide|anywhere in the world|global remote/.test(t)) {
    return { japan_eligible: true, confidence: 0.7, eligible_region: 'Worldwide', evidence: 'Explicit worldwide/global remote signal.' };
  }
  if (/us only|united states only|must reside in the us|u\.s\. only/.test(t)) {
    return { japan_eligible: false, confidence: 0.9, eligible_region: 'US only', evidence: 'Explicit US-only restriction.' };
  }
  return {
    japan_eligible: country.toUpperCase() === 'JP' ? true : null,
    confidence: country.toUpperCase() === 'JP' ? 0.8 : 0.2,
    eligible_region: country || null,
    evidence: country.toUpperCase() === 'JP' ? 'Country field is JP.' : 'No reliable Japan-eligibility evidence.'
  };
};
...
const response = await fetch(FEED_URL, {
  headers: { 'user-agent': 'GlobalWorkRadar/0.2' }
});
if (!response.ok) throw new Error(`Workable feed fetch failed: ${response.status} ${response.statusText}`);

const xml = await response.text();
if (!xml.includes('<source') || !xml.includes('<job>')) throw new Error('Workable feed failed structural validation.');
...
for (const match of xml.matchAll(/<job>([\s\S]*?)<\/job>/gi)) {
  ...
  const city = tag(item, 'city');
  const state = tag(item, 'state');
  const country = tag(item, 'country');
  const remoteRaw = tag(item, 'remote').toLowerCase();
  const remote = remoteRaw === 'true';
  const description = stripHtml(tag(item, 'description'));
  const jobType = tag(item, 'jobtype');
  const sourceCategory = tag(item, 'category');
  const experience = tag(item, 'experience');
  ...
  const record = {
    id,
    canonical_key: sha(`${company.toLowerCase()}|${title.toLowerCase()}|${url}`).slice(0, 24),
    ...
    title,
    location,
    eligible_region: eligibility.eligible_region,
    eligibility_evidence: eligibility.evidence,
    remote_type: remote ? 'remote' : 'unknown',
    japan_eligible: eligibility.japan_eligible,
    japan_eligibility_confidence: eligibility.confidence,
    ...
    category: classify(title, sourceCategory, description),
    published_at: iso(tag(item, 'date')),
    ...
    source_policy: 'feed',
    source_attribution: 'Workable',
    quality_score: eligibility.japan_eligible === true ? 0.85 : 0.65,
    publishable: eligibility.japan_eligible === true || japaneseRequired,
...
const historyEvents = records
  .filter((record) => record.change_type !== 'unchanged')
  .map((record) => ({
    job_id: record.id,
...
for (const [id, prev] of Object.entries(previousJobs)) {
  ...
  if (missingCount >= 2) {
    closedNow += prev.closed_at ? 0 : 1;
    nextStateJobs[id] = { ...prev, missing_count: missingCount, closed_at: prev.closed_at || observedAt };
    if (!prev.closed_at) {
      historyEvents.push({
        job_id: id,
        observed_at: observedAt,
    ...
    nextStateJobs[id] = { ...prev, missing_count: missingCount };
    historyEvents.push({
      job_id: id,
      observed_at: observedAt,
      verification_status: 'unknown',
      fingerprint: prev.fingerprint,
      change_type: 'missing_once'
    });
  }
}
...
for (const job of records) employerCounts.set(job.employer, (employerCounts.get(job.employer) || 0) + 1);
const recurringEmployers = [...employerCounts.values()].filter((count) => count > 1).length;
const survivalDays = records
  .map((j) => (new Date(observedAt) - new Date(j.first_seen_at)) / 86400000)
...
const market = {
  ...
  active_jobs: active,
  japan_eligible_jobs: japanEligible,
  japanese_jobs: japanese,
  remote_jobs: remote,
  new_jobs_since_previous_snapshot: newJobs,
  closed_jobs_confirmed_this_snapshot: closedNow,
  remote_ratio: active ? remote / active : 0,
  employer_recurrence_count: recurringEmployers,
  median_observed_survival_days: median(survivalDays),
  intelligence_status: previousState.last_observed_at ? 'history_available' : 'baseline_only'
};
...
if (historyEvents.length) await appendFile(HISTORY_FILE, historyEvents.map((event) => JSON.stringify(event)).join('\n') + '\n');
