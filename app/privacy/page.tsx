...
export default function PrivacyPage() {
  return <main className="shell" style={{maxWidth:860,paddingTop:72,paddingBottom:96}}>
    ...
    <p style={{lineHeight:1.9,opacity:.78}}>Vector Praxisは、どの入口やCTAが実際に使われているかを判断するために、PostHogによる最小限のファネル計測を利用します。検索語や決済情報そのものをVector Praxis Hubの分析イベントへ送る設計にはしていません。</p>

    <section style={{marginTop:42}}>
      <h2>このHubで計測するもの</h2>
      <ul style={{lineHeight:1.9,opacity:.78}}>
        <li>ページのパス、参照元、UTMなどの流入情報</li>
        <li>Hub内の明示的な導線・CTA・外部リンクが利用されたこと</li>
        <li>Revenue Routeを区別するためのasset_id / route_idなどの識別子</li>
      </ul>
      <p style={{lineHeight:1.9,opacity:.78}}>PostHogは自動クリック収集を無効化し、セッション録画を無効化し、永続的なPerson Profileを作らない設定で利用します。Web上の別トラッカーを重ねず、Revenue Funnelの判断に必要な明示イベントだけを送る構成です。接続時には、HTTP通信に必要な技術情報がサービス側で処理される場合があります。</p>
    </section>

    <section style={{marginTop:36}}>
      <h2>Global Privacy Control / Do Not Track</h2>
      <p style={{lineHeight:1.9,opacity:.78}}>ブラウザがGlobal Privacy Control（GPC）またはDo Not Trackを有効にしている場合、このHubではPostHogの計測スクリプトを読み込まないようにしています。</p>
    </section>

    <section style={{marginTop:36}}>
      <h2>Vector内のBing検索</h2>
      <p style={{lineHeight:1.9,opacity:.78}}>AI Business Pulseの検索欄は通常のHTMLフォームでBingへ直接送信します。入力した検索語はVector Praxisのサーバーで処理・保存せず、Vector Praxisの分析イベントにも含めません。Vector側では「Bing検索が使われた」というイベントだけを記録できます。送信後はBing側の利用条件・プライバシー方針が適用されます。</p>
    </section>
